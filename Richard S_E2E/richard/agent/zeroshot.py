"""Module for the zero shot agent. Optimized for gemini-1.5-pro use."""
import types
...
def get_zeroshot_agent(llm: ChatGoogleGenerativeAI, verbose: bool = False) -> AgentExecutor:
    """Get the zero shot agent. Optimized for gemini-1.5-pro use."""
...
def _get_full_inputs(
    ...
    **kwargs: Dict[str, Any]
    ...
    )  # limit to 4 intermediate steps for gemini-1.5-pro
...
class ZeroShotAgent(ChromeGPTAgent):
    def __init__(self, model: str = "gemini-1.5-pro", verbose: bool = False) -> None:
        """Initialize the ZeroShotAgent."""
        self.model = model
        self.agent = get_zeroshot_agent(
            llm=ChatGoogleGenerativeAI(model=self.model, temperature=0, google_api_key="AIzaSyAU97ZxASlUERhrK3MJ2IlIEYLL5lzxm4Y"),  # type: ignore
            verbose=verbose,
        )
...
class BabyAGIAgent(ChromeGPTAgent):
    ...
    def _get_zero_shot_agent(
        self, llm: ChatGoogleGenerativeAI, verbose: bool, tools: List[BaseTool]
    ) -> AgentExecutor:
        prefix = (
