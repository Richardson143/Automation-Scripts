...
class AutoGPTPrompt(BaseChatPromptTemplate):
    ...
    tools: List[BaseTool] = Field(..., description="List of tools available to the AI")
    token_counter: Callable[[str], int] = Field(..., description="Function to count tokens")
    send_token_limit: int = Field(4196, description="Maximum number of tokens to send in a single prompt")
